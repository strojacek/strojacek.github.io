...
Alright, we have the visitor, now what?

Well, we want to look at the structure of the data that we are going to work in.

Dap/SAS primarily operates on the notion that everything is a data frame (though Dap/SAS isn't the only language that operates on the principle, R does as well).

What is a data frame? From a very high level, a data frame is nothing more than a table. Same as what you see in a spreadsheet. Also note, this is basically one and the same as a vector in linear algebra.

So as an example, let's say we have the following data:
...
to implement this within R, we could do something like: 
...
However, in Dap/SAS we would need to do the following:
...
So essentially, we want to do the following:
